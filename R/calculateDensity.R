...
toc();
	...
	if (genomeWideCutoff) {
		...
		cat("Storing density sample for genome-wide cutoff...\t");
		sampleFileName = paste(scratchDir, chrom, ".sample.", windowSize, "-", windowStep, ".wig", sep="")
		file.create(sampleFileName);
		cwriteStep(as.vector(densityEstimate), as.numeric(length(densityEstimate)), file=sampleFileName, step=10, showWarnings=FALSE); toc();
	}
